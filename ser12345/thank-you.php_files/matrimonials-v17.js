<!--
function clear_textarea(personalmessage)
{
	var text_val=personalmessage.personalisedmessage.value;
 	var search_dtxt = text_val.search("Type your message here. For example,");
	if(search_dtxt>=0){	personalmessage.personalisedmessage.value =''}
}

function calcCharLen1(sForm, sTextArea, sTextInput, iMaxLimit)
{
	var _oDF        = sForm; //document.forms[sForm];
...
function calcCharLen(sForm, sTextArea, sTextInput, iMaxLimit)
{
	...
	if(sForm == "profile" && sTextArea == "aboutyourself" && _iCharLeft > 100)
	{
		abc = false;
		toggleHint('hide', "aboutyourself");
	}
	else
	{
		abc = true;
		toggleHint('show', "aboutyourself");
	}
...
function chk_community(pg){

	var obj_community = document.quicksearch.community;
	var relcom = "Community";
	if (obj_community.options[obj_community.selectedIndex].value == "")
	{
		
		if(pg=="religion") 
			relcom = "Religion";
		alert("Please select "+relcom);
		obj_community.focus();
		return false;
	}else{
		return true;
...
//PSL_EN_1 start
/**
 * check whether religion is selected in the quick search form
 *
 * @author Madhuvanti
 * @param  
 * 
 * @usage  return	chk_religion();
 */
function chk_religion(){
	if (document.quicksearch.religion.options[document.quicksearch.religion.selectedIndex].value == ""){
 ...
 * @param  
 * 
 * @usage  onSubmit="return chk_quicksearch_religion();"
 */

function chk_quicksearch_religion(){	
...
function chk_quicksearch(pg){
	if (age_diff_max() == true){
		return	chk_community(pg);
	}
	else{
		return false;
	}
...
//PSL_EN_1 start
/**
 * set the community field value in quick search box based on the religion value
 *
 * @author Madhuvanti
 * @param  int	selectedReligion
 * 
 * @usage  <select name="religion" style="width:142px" onChange="set_community_value(this.selectedIndex)">

*/
function set_community_value(selectedReligion)
{
	if ((selectedReligion == 0) || (selectedReligion == 1) ) 
	{
		document.getElementById('loadCommunity').innerHTML = 
		"<select name=\"community\" style=\"width:142px\" onchange=\"set_comm_val(this.value)\" class=\"formselect\"  id=\"community\" disabled>"+
		"<option value=\" \">Doesn't Matter</option>"+
		"</select>";
		document.getElementById('hiddenCommunity').value="";
	}
	else if (selectedReligion == 2) 
	{
		document.getElementById('loadCommunity').innerHTML = 
		"<select name=\"community\" onchange=\"set_comm_val(this.value)\" class=\"formselect\"  style=\"width:142px\" id=\"community\">"+
		"<option value=\"Hindu\">All</option>"+
		"<option value=\"Hindu|Assamese|Arunachali|Manipuri\">Assamese</option>"+
		"<option value=\"Hindu|Bengali\">Bengali</option>"+
		"<option value=\"Hindu|Gujarati|Kutchi\">Gujarati</option>"+
		"<option value=\"Hindu|Hindi|Awadhi|Bhojpuri|Brij|Chattisgarhi|Garhwali|Haryanavi|Himachali/Pahari|Kumaoni|Magahi|Maithili|Rajasthani|Sanskrit|Urdu\">Hindi</option>"+
		"<option value=\"Hindu|Kannada|Coorgi|Tulu\">Kannada</option>"+
		"<option value=\"Hindu|Konkani\">Konkani</option>"+
		"<option value=\"Hindu|Malayalam\">Malayalam</option>"+
		"<option value=\"Hindu|Marathi\">Marathi</option>"+
		"<option value=\"Hindu|Marwari|Rajasthani\">Marwari</option>"+
		"<option value=\"Hindu|Oriya\">Oriya</option>"+
		"<option value=\"Hindu|Punjabi|Dogri\">Punjabi</option>"+
		"<option value=\"Hindu|Sindhi\">Sindhi</option>"+
		"<option value=\"Hindu|Tamil\">Tamil</option>"+
		"<option value=\"Hindu|Telugu\">Telugu</option>"+
		"<option value=\"Hindu|Other|Aka|Arabic|Arunachali|Awadhi|Bhojpuri|Bhutia|Burmese|Chattisgarhi|Chinese|English|French|Garhwali|Garo|Haryanavi|Himachali/Pahari|Hindko|Kakbarak|Kanauji|Kashmiri|Khandesi|Khasi|Koshali|Kumaoni|Ladakhi|Lepcha|Magahi|Maithili|Malay|Manipuri|Miji|Mizo|Monpa|Nepali|Pashto|Persian|Russian|Sanskrit|Santhali|Sinhala|Spanish|Swedish|Tagalog|Urdu\">Other</option>"+		
		"</select>";
		document.getElementById('hiddenCommunity').value="Hindu";
	}
	else if (selectedReligion == 3) 
	{
		document.getElementById('loadCommunity').innerHTML = 
		"<select name=\"community\" onchange=\"set_comm_val(this.value)\" class=\"formselect\"   style=\"width:142px\" id=\"community\">"+
		"<option value=\"Muslim\">All</option>"+
		"<option value=\"Muslim:Sunni\">Sunni</option>"+
		"<option value=\"Muslim:Shia\">Shia</option>"+
		"<option value=\"Muslim:Rajput\">Rajput</option>"+
		"<option value=\"Muslim:Dawoodi Bohra\">Dawoodi Bohra</option>"+
		"<optgroup label=\"-----------------------\"></optgroup>"+
		"<option value=\"Muslim|Urdu\">Urdu</option>"+
		"<option value=\"Muslim|Punjabi\">Punjabi</option>"+
		"<option value=\"Muslim|Bengali\">Bengali</option>"+
		"<option value=\"Muslim|Hindi\">Hindi</option>"+
		"<option value=\"Muslim|English\">English</option>"+
		"</select>";
		document.getElementById('hiddenCommunity').value="Muslim";
	}
	else if (selectedReligion == 4) 
	{
		document.getElementById('loadCommunity').innerHTML = 
		"<select name=\"community\"  onchange=\"set_comm_val(this.value)\" class=\"formselect\"  style=\"width:142px\" id=\"community\">"+
		"<option value=\"Christian\">All</option>"+
		"<option value=\"Christian:Roman Catholic\">Roman Catholic</option>"+
		"<option value=\"Christian:Protestant\">Protestant</option>"+
		"<option value=\"Christian:Born Again\">Born Again</option>"+
		"<option value=\"Christian:Marthoma\">Marthoma</option>"+
		"<optgroup label=\"-----------------------\"></optgroup>"+
		"<option value=\"Christian|Malayalam\">Malayalam</option>"+
		"<option value=\"Christian|English\">English</option>"+
		"<option value=\"Christian|Tamil\">Tamil</option>"+
		"<option value=\"Christian|Konkani\">Konkani</option>"+
		"</select>";
		document.getElementById('hiddenCommunity').value="Christian";
	}
	else if (selectedReligion == 5) 
	{
		document.getElementById('loadCommunity').innerHTML = 
		"<select name=\"community\"  onchange=\"set_comm_val(this.value)\" class=\"formselect\"  style=\"width:142px\" id=\"community\">"+
		"<option value=\"Sikh\">All</option>"+
		"<option value=\"Sikh:Jat\">Jat</option>"+
		"<option value=\"Sikh:Ramgharia\">Ramgharia</option>"+
		"<option value=\"Sikh:Khatri\">Khatri</option>"+
		"<option value=\"Sikh:Gursikh\">Gursikh</option>"+
		"<option value=\"Sikh:Clean Shaven\">Clean Shaven</option>"+
		"<optgroup label=\"-----------------------\"></optgroup>"+
		"<option value=\"Sikh|Punjabi\">Punjabi</option>"+
		"<option value=\"Sikh|English\">English</option>"+
		"</select>";
		document.getElementById('hiddenCommunity').value="Sikh";
	}
	else if (selectedReligion == 6) 
	{
		document.getElementById('loadCommunity').innerHTML = 
		"<select name=\"community\"  onchange=\"set_comm_val(this.value)\" class=\"formselect\"  style=\"width:142px\" id=\"community\">"+
		"<option value=\"Jain\">All</option>"+
		"<option value=\"Jain:Shwetamber\">Shwetamber</option>"+
		"<option value=\"Jain:Digambar\">Digambar</option>"+
		"<option value=\"Jain:Vania\">Vania</option>"+		
		"</select>";
		document.getElementById('hiddenCommunity').value="Jain";
	}
	else if (selectedReligion == 7) 
	{
		document.getElementById('loadCommunity').innerHTML = 
		"<select name=\"community\"  onchange=\"set_comm_val(this.value)\" class=\"formselect\"  style=\"width:142px\" id=\"community\" disabled>"+
		"<option value=\"Parsi\">All</option>"+	
		"</select>";
		document.getElementById('hiddenCommunity').value="Parsi";
	}
	else if (selectedReligion == 8) 
	{
		document.getElementById('loadCommunity').innerHTML = 
		"<select name=\"community\"  onchange=\"set_comm_val(this.value)\" class=\"formselect\"  style=\"width:142px\" id=\"community\" disabled>"+
		"<option value=\"Buddhist\">All</option>"+	
		"</select>";
		document.getElementById('hiddenCommunity').value="Buddhist";
	}
	else if (selectedReligion == 9) 
	{
		document.getElementById('loadCommunity').innerHTML = 
		"<select name=\"community\"  onchange=\"set_comm_val(this.value)\" class=\"formselect\"  style=\"width:142px\" id=\"community\" disabled>"+
		"<option value=\"Jewish\">All</option>"+	
		"</select>";
		document.getElementById('hiddenCommunity').value="Jewish";
	}
	else if (selectedReligion == 10) 
	{
		document.getElementById('loadCommunity').innerHTML = 
		"<select name=\"community\"  onchange=\"set_comm_val(this.value)\" class=\"formselect\"  style=\"width:142px\" id=\"community\" disabled>"+
		"<option value=\"No Religion\">All</option>"+	
		"</select>";
		document.getElementById('hiddenCommunity').value="No Religion";
	}
	else if (selectedReligion == 11) 
	{
		document.getElementById('loadCommunity').innerHTML = 
		"<select name=\"community\"  onchange=\"set_comm_val(this.value)\" class=\"formselect\"  style=\"width:142px\" id=\"community\" disabled>"+
		"<option value=\"Spiritual - not religious\">All</option>"+	
		"</select>";
		document.getElementById('hiddenCommunity').value="Spiritual - not religious";
	}
	else if (selectedReligion == 12) 
	{
		document.getElementById('loadCommunity').innerHTML = 
		"<select name=\"community\"  onchange=\"set_comm_val(this.value)\" class=\"formselect\"  style=\"width:142px\" id=\"community\" disabled>"+
		"<option value=\"Other\">All</option>"+	
		"</select>";
		document.getElementById('hiddenCommunity').value="Other";
	}
}


function set_comm_val(val)
{
	document.getElementById('hiddenCommunity').value=val;
}
//PSL_EN_1 end
